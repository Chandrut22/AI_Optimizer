...
# Reducer to handle multiple nodes writing to error_message simultaneously
def reduce_error_messages(left: Optional[str], right: Optional[str]) -> str:
    if not left:
        return right or ""
    if not right:
        return left
...
class AgentState(TypedDict):
  ...
  # Added Annotated reducer to handle parallel errors
  error_message: Annotated[Optional[str], reduce_error_messages]

def call_report_generator(state: AgentState):
    """Node that generates the final markdown report."""
    if not state.get("strategy_result"):
        return {"error_message": "Cannot generate report: SEO Strategy is missing."}
...
def call_seo_optimizer(state: AgentState):
    ...
    try:
        ...
        if optimization_result.get("error_message"):
...
def call_seo_strategist(state: AgentState):
    ...
    try:
        ...
        if strategy_result.get("error_message"):
...
def call_market_researcher(state: AgentState):
    ...
    try:
        ...
        if market_research_report.get("error_message"):
...
def call_on_page_analyzer(state: AgentState):
    ...
    if not url:
        return {"error_message": "URL not provided in state for on-page analysis."}
    if not html_content:
        return {"error_message": "HTML content not available in state for on-page analysis."}
...
def call_technical_auditor(state: AgentState):
    ...
    url = state["url"]
    if not url:
        return {"error_message": "URL not provided in state for technical audit."}
...
def call_firecrawl(state: AgentState):
    ...
    url = state["url"]
    if not url:
        return {"error_message": "URL not provided in state."}

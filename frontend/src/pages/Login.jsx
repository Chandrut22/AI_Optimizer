...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // returns user object combined with 'hasSelectedTier' boolean
      const userData = await loginUser(data.email, data.password); 
      
      // Step 2: Update Auth Context
      setUser(userData); 

      // Step 3: Navigate based on tier selection
      if (userData.hasSelectedTier) {
        navigate("/dashboard");
      } else {
        navigate("/pricing");
      }
        ...
        <div className="w-full max-w-md">
          <div className="bg-[#FFFFFF] dark:bg-[#1E293B] border border-[#D1D5DB] dark:border-[#475569] rounded-xl shadow-lg p-8">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="email" className="text-sm font-medium text-[#111827] dark:text-[#F8FAFC]">
                  Email address
                </Label>
                <Input
                  id="email"
                  ...
                  className={cn(
                    ...
                    (errors.email || loginError) && "border-red-500 focus:border-red-500 focus:ring-red-500/20"
                  )}
                  {...register("email")}
                />
                {errors.email && (
              ...
              <div className="space-y-2">
                <Label htmlFor="password" className="text-sm font-medium text-[#111827] dark:text-[#F8FAFC]">
                  Password
                </Label>
                ...
                <div className="flex justify-end">
                  <Link to="/forgot-password" className="text-sm text-[#38BDF8] hover:text-[#38BDF8]/80 transition-colors">
                    Forgot password?
                  </Link>
                </div>
              </div>
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-[#6B7280] dark:text-[#94A3B8]">
                ...
                <Link to="/register" className="text-[#38BDF8] font-medium hover:text-[#38BDF8]/80 transition-colors">
                  Sign up
                </Link>
              </p>
            </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-xs text-[#6B7280] dark:text-[#94A3B8]">
              Need help?{" "}
              <Link to="/contact" className="text-[#38BDF8] hover:underline transition-colors">
                Contact support
              </Link>{" "}
              or{" "}
              <Link to="/verify-email" className="text-[#38BDF8] hover:underline transition-colors">
                verify your email
              </Link>
            </p>
          </div>
